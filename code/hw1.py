...
def main():
    ...
    image1 = cv2.cvtColor(cv2.imread(os.path.join(main_path, 'data', '2.png')), cv2.COLOR_BGR2RGB)
    image2 = cv2.cvtColor(cv2.imread(os.path.join(main_path, 'data', '1.png')), cv2.COLOR_BGR2RGB)
    ...
    if image1.shape != image2.shape:
        ...
        _h = min(h1, h2)
        _dim = (_w, _h)
        image1 = cv2.resize(image1, _dim, interpolation = cv2.INTER_AREA)
        image2 = cv2.resize(image2, _dim, interpolation = cv2.INTER_AREA)
    ...
    #plt.imsave(os.path.join(main_path, 'results', 'low_frequencies_.png'), low_frequencies, dpi=95)
    #plt.imsave(os.path.join(main_path, 'results', 'high_frequencies.png'), normalize(high_frequencies + 0.5), dpi=95)
    plt.imsave(os.path.join(main_path, 'results', 'hybrid_heisenberg_image.png'), hybrid_image, dpi=95)
    plt.imsave(os.path.join(main_path, 'results', 'hybrid_heisenberg_scales.png'), hybrid_image_scales, dpi=95)
